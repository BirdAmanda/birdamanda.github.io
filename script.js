...
/* This function tests to see if a specific player has won */
...
/* This function is to test if the human or computer wins */
...
/* *** AI function that choice the best move. Brilliant AI! Good Job *** */

function tictactest(state, depth, player) {
	var best;
	...
	emptyCells(state).forEach(function (cell) {
		...
		var y = cell[1];
		state[x][y] = player;
		var score = tictactest(state, depth - 1, -player);
		state[x][y] = 0;
		score[0] = x;
		score[1] = y;
...
/* It calls the tictactest function */
function aiTurn() {
	var x, y;
	var move;
	var cell;
	...
	if (emptyCells(board).length == 9) {
		...
		move = tictactest(board, emptyCells(board).length, COMP);
		x = move[0];
		y = move[1];
	}
...
/* Restart the game by using the begin AI or retry button*/
function restartBnt(button) {
	if (button.value == "Begin AI") {
		aiTurn();
		button.disabled = true;
	}
	else if (button.value == "Retry") {
		var htmlBoard;
		var msg;
